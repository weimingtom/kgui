...
class kGUISystemWin : public kGUISystem
{
public:
...
void kGUISystemWin::Draw(HWND hWnd,int ulx,int urx,int uty,int uby)
{
	int sw,sh;
	int dw,dh;
	int nl;
...
void kGUISystemWin::FileShow(const char *filename)
{
	ShellExecute(NULL,"open",filename,NULL,NULL,SW_SHOWNORMAL);
}

void kGUISystemWin::ShellExec(const char *program,const char *parms,const char *dir)
{
...
bool kGUISystemWin::MakeDirectory(const char *name)
{
	if(_mkdir(name)==0)
		return(true);
	return(false);
...
void kGUISystemWin::Copy(kGUIString *s)
{
	HGLOBAL hglbCopy; 
    LPTSTR  lptstrCopy; 
	kGUIString u;
...
void kGUISystemWin::ReDraw(void)
{
	if(m_showwindow==false)
		ShowWindow();
...
void kGUISystemWin::ChangeMouse(void)
{
	...
	SetCursor(AfxGetApp()->LoadStandardCursor(mousecursors[kGUI::GetMouseCursor()]));
}

void kGUISystemWin::ShowMouse(bool show)
{
	ShowCursor(show);
}

void kGUISystemWin::GetWindowPos(int *x,int *y,int *w,int *h)
{
	WINDOWPLACEMENT    wp;
...
void kGUISystemWin::AdjustMouse(int dx,int dy)
{
	m_mousex+=dx;
	m_mousey+=dy;
}

void kGUISystemWin::SetWindowPos(int x,int y)
{
	WINDOWPLACEMENT    wp;
	int w,h;
...
void kGUISystemWin::SetWindowSize(int w,int h)
{
	WINDOWPLACEMENT    wp;
...
void kGUISystemWin::ShowWindow(void)
{
	m_showwindow=true;
	::ShowWindow(m_hWnd,SW_SHOW);
}

void kGUISystemWin::HideWindow(void)
{
	m_showwindow=false;
	::ShowWindow(m_hWnd,SW_HIDE);
}

void kGUISystemWin::Minimize(void)
{
	::ShowWindow(m_hWnd,SW_MINIMIZE);
}

void kGUISystemWin::Sleep(int ms)
{
	::Sleep(ms);
}

/* is this a directory? */
bool kGUISystemWin::IsDir(const char *fn)
{
	bool isdir;
	intptr_t dir_handle;
...
void kGUISystemWin::GetPrinters(void)
{
	DWORD numprinters;
	DWORD defprinter=0;
...
bool kGUIPrintJobWin::Start(kGUIPrinter *p,const char *jobname,int numpages,int numcopies,double ppi,double lm,double rm,double tm,double bm,kGUIDrawSurface *surface,bool landscape)
{
	int ppw;
	int pph;
	double paiw,paih;
	...
	ppw = m_pdc.GetDeviceCaps(HORZRES);
	pph = m_pdc.GetDeviceCaps(VERTRES);

	/* get printer pixels per inch */	
	m_ppiw = m_pdc.GetDeviceCaps(LOGPIXELSX);
	m_ppih = m_pdc.GetDeviceCaps(LOGPIXELSY);

	/* get physical unprintable margins on printer in inches */
	hardleft = (double)m_pdc.GetDeviceCaps(PHYSICALOFFSETX)/(double)m_ppiw;
	hardtop  = (double)m_pdc.GetDeviceCaps(PHYSICALOFFSETY)/(double)m_ppih;
...
void kGUIPrintJobWin::StartPage(void)
{
...
void kGUIPrintJobWin::DrawRect(int lx,int rx,int ty,int by,kGUIColor color)
{
	int r,g,b;
	HBRUSH hBrush;
	RECT  rect;
	...
	FillRect(m_pdc,&rect,hBrush);
	DeleteObject(hBrush);
}

void kGUIPrintJobWin::DrawTextList(int pointsize,const char *fontname,int num, Array<PRINTFONTCHAR_DEF> *list)
{
	int i;
	int r,g,b;
	int oldbg;
	...
	oldbg=m_pdc.SetBkMode(TRANSPARENT);
	...
	for(i=0;i<num;++i)
	{
		...
		c=pfc.c;
		DrawText(m_pdc,&c,1,&rect,DT_NOPREFIX);
	}

	m_pdc.SetBkMode(oldbg);
...
void kGUIPrintJobWin::DrawImage(int lx,int rx,int ty,int by)
{
	int dlx,drx,dty,dby;
	BITMAPINFO bmpi;
