...
public:
	...
	/*! compare callback 
		...
		@param f function pointer
	    @return true=is a match, false= not a match */
	bool Is(void *o,void (*f)(void *,T *)) {if((o==m_obj) && (m_func==f))return true;return false;}
private:
	void *m_obj;
	void (*m_func)(void *,T *);
};
...
public:
	...
	const unsigned int GetWidest(void);
...
EVENT_TRYMOVED,				/* user tried to move tab but tab was locked (used to display warnings) */
...
EVENT_MOUSEOVER,			/* used for images */
EVENT_DESTROY,				/* called just before delete for all kGUIObj derived objects */
...
/* trigerred by the input box after a change or nochange event if the user pressed return */
EVENT_PRESSRETURN,			
...
public:
	...
	bool SaveJPGImage(DataHandle *outdh,int quality);
	bool SavePNGImage(DataHandle *outdh);
...
/* any object that can attach to the search replace */
/* object needs to be derived from this class */
...
/*! @class kGUIInputBoxObj
	...
	@ingroup kGUIObjects */
class kGUIInputBoxObj : public kGUIObj, public kGUIText, public kGUISearchReplaceObj
{
public:
	kGUIInputBoxObj();
	~kGUIInputBoxObj();
	...
	void SetAllowFind(bool b) {m_allowfind=b;}
	void SetInt(int v);
	...
	unsigned int GetXOffset(void) {return m_xoff;}
...
public:
	...
	void SetShowIterators(bool i) {m_showiterators=i;}
private:
...
public:
	...
	void SetShowIterators(bool i) {m_showiterators=i;}
	bool GetShowIterators(void) {return m_showiterators;}
private:
	unsigned int m_colorcolwidth;
...
public:
	...
	void SetLoop(bool l) {m_loop.SetSelected(l);}
...
public:
	...
	static void DrawBox(int x1,int y1,int x2,int y2,kGUIColor color);
	...
	static void DrawPixel(int x,int y,kGUIColor c,double alpha);
	...
	static bool InsideClip(int lx,int ty,int rx,int by);
	static CLIPOUT_DEF GetClipOutCode(int x,int y);
	static CLIPOUT_DEF GetClipOutCodeR(int r,int x,int y);
	...
	static int LoadFont(const char *filename,bool bold=false);
...
//	static bool PointInsidePoly(float px,float py,int nvert,kGUIFPoint2 *point);
	...
	/* update tasks */
	/*! Add task to update task list called once per UpdateInput
	    @param codeobj pointer to object
		@param code pointer to function */
	static void AddUpdateTask(void *codeobj,void (*code)(void *));
	/*! Remove task from update task list
	    @param codeobj pointer to object
		@param code pointer to function */
	static void DelUpdateTask(void *codeobj,void (*code)(void *));

	/*! Add global event listener
	    @param codeobj pointer to object
		@param code pointer to function */
	static void AddEventListener(void *codeobj,void (*code)(void *,kGUIEvent *e));
	/*! Remove global event listener
	    @param codeobj pointer to object
		@param code pointer to function */
	static void DelEventListener(void *codeobj,void (*code)(void *,kGUIEvent *e));

	static void CallEventListeners(kGUIEvent *e);
...
//	static void SetScreenSize(int w,int h) {m_screenwidth=w;m_screenheight=h;}
	...
	//used for debugging!
	static int GetClipIndex(void) {return m_clipindex;}
...
private:
	...
	static unsigned int m_numupdatetasks;
	static Array<kGUICallBack *>m_updatetasks;

	static unsigned int m_numeventlisteners;
	static Array< kGUICallBackPtr<kGUIEvent> *>m_eventlisteners;
