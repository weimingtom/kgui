...
*/
	conn = (sslConn_t *)calloc(sizeof(sslConn_t), 1);
	conn->fd = fd;
...
readMore:
	rc = sslRead(conn, (char *)buf, sizeof(buf), &status);
/*
...
*/
	cp->inbuf.start = cp->inbuf.end = cp->inbuf.buf = (unsigned char *)malloc(len);
	cp->inbuf.size = len;
/*
...
void sslWriteClosureAlert(sslConn_t *cp)
{
	if (cp != NULL) {
		...
		setSocketNonblock(cp->fd);
		send(cp->fd, (const char *)cp->outsock.start,
			(int)(cp->outsock.end - cp->outsock.start), MSG_NOSIGNAL);
	}
}
...
int psSocketWrite(SOCKET sock, sslBuf_t *out)
{
	...
	while (out->start < out->end) {
		bytes = send(sock, (const char *)out->start, (int)(out->end - out->start), MSG_NOSIGNAL);
		if (bytes == SOCKET_ERROR) {
			return -1;
		}
		out->start += bytes;
