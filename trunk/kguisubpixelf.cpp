...
kGUISubPixelCollectorF::kGUISubPixelCollectorF()
{
	m_lines.Init(2048,256);
	m_chunks.SetBlockSize(65536);
}

void kGUISubPixelCollectorF::SetBounds(float y1,float y2)
{
	int y;
	SUBLINEF_DEF *list;
...
void kGUISubPixelCollectorF::SetColor(kGUIColor c,float alpha)
{
	int r,g,b;
...
void kGUISubPixelCollectorF::AddRect(float x,float y,float w,float h,float weight)
{
	int ty,by;
	float rx,th;

	rx=x+w;
	if(x<kGUI::m_clipcornersf.lx)
		x=kGUI::m_clipcornersf.lx;
	if(rx>kGUI::m_clipcornersf.rx)
		rx=kGUI::m_clipcornersf.rx;
	if(rx<=x)
		return;	/* off */
...
void kGUISubPixelCollectorF::AddChunk(int y,float lx,float rx,float weight)
{
	int lineindex;
	SUBLINEPIXF_DEF *chunk;
	SUBLINEF_DEF *line;
	SUBLINEPIXF_DEF *prev;
	...
	chunk=(SUBLINEPIXF_DEF *)m_chunks.Alloc(sizeof(SUBLINEPIX_DEF));
	chunk->next=prev;
	chunk->weight=weight;
	chunk->leftx=lx;
	chunk->width=rx-lx;
...
void kGUISubPixelCollectorF::Draw(void)
{
	int br,bg,bb;
	int newr,newg,newb;
	SUBLINEF_DEF *lines;
	...
	//int gindex;
	float m_weights[2048];		/* hmm, this should not really be hardcoded */
	float weight,bweight,width,fwidth;
	kGUIColor *cp;
...
//					assert(m_weights[clx]<=1.0f,"Overflow!");
				...
				{
					fwidth=1.0f-(chunk->leftx-(float)clx);
