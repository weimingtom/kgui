...
/* this is generated by kguilocstr from the text in kguiloc.txt */
#include "_text.cpp"
...
void kGUI::UpdateInput(void)
{
	...
	if(m_activeobj)
	{
		...
		do
		{
			...
			if(m_activeobj->UpdateInput()==true || m_forceused)
				goto used;
...
void kGUILocStrings::Init(LOCSTRING_DEF *loc)
{
	...
	m_numlangs=loc->numlangs;
	m_numstrings=loc->numstrings;
	
	total=m_numlangs*m_numstrings;
	m_strings=new kGUIString[total];
	for(i=0;i<total;++i)
	{
		m_strings[i].SetString((const char *)loc->strings[i]);
		m_strings[i].SetEncoding(ENCODING_UTF8);
	}
}
