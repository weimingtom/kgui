...
//	movesize=(((listsize+1)-numentries)-num)*sizeof(T);
...
public:
	...
	/*! copy the font info from another instance */
//	void CopyFontInfo(kGUIFontInfo *fi) {m_fontid=fi->m_fontid;m_size=fi->m_size;m_color=fi->m_color;}
private:
...
EVENT_SELECTED,				/* menu selection was selected */
EVENT_CANCELLED,			/* menu selection was aborted */
EVENT_CLOSE,				/* window was closed */
EVENT_SIZECHANGED,			/* window size was changed */
...
private:
	CALLBACKGLUE(kGUIComboBoxObj,SelectionDone)
	void SelectionDone(void);
	void DrawPopUp(void);
...
public:
	...
	const char *GetSelectionString(void) {return GetSelectionStringObj()->GetString();}
private:
	CALLBACKGLUE(kGUISharedComboboxObj,SelectionDone)
	void SelectionDone(void);
	void DrawPopUp(void);
	bool m_locked:1;
	bool m_popped:1;		/* selector list is currently shown */
...
public:
	...
	CALLBACKGLUE(kGUIMenuColObj,SelectionDone)
private:
	void SelectionDone(void);
	void Resize(void);
	void DrawPopUp(void);
...
public:
	...
	void SetTabName(int tabindex,const char *name) {m_tabnames.GetEntry(tabindex)->SetString(name);UpdateTabs();}
	const char *GetTabName(int index) {return m_tabnames.GetEntry(index)->GetString();}
	kGUIText *GetTabTextPtr(int index) {return m_tabnames.GetEntry(index);}
...
public:
	...
	inline void PopRowHeaders(void) {m_poprowheaders=true;CalcChildZone();}
	...
	//easy way to save user adjusted column widths and hide/show into a xml file
	...
	/* setnew func can also be used for loading from database tables and not allow user to add rows if desired */
	void SetSelectedCallBack(void *codeobj,void (*code)(void *)) {m_selcallback.Set(codeobj,code);}
...
private:
	...
	/* called when a combo box entry is selected */
	kGUICallBack m_selcallback;
...
//	inline void SetArea(int w,int h) {SetSize(w+WEDGE+WEDGE,h+WTOP+WEDGE);}
...
//	virtual int GetTableColHeaderHeight(void)=0;
...
enum
{
//these are for the user application to use
KGUISTRING_LANGUAGE,
KGUISTRING_ENGLISH,
KGUISTRING_FRENCH,
...
KGUISTRING_DONE,
KGUISTRING_TODAY,
//these are for the print preview page
KGUISTRING_PRINTPREVIEW,
KGUISTRING_PRINT,
KGUISTRING_CURRENT,
KGUISTRING_PAGE,
KGUISTRING_ZOOM,
KGUISTRING_PORTRAIT,
KGUISTRING_LANDSCAPE,
KGUISTRING_SCALEPERCENT,
KGUISTRING_MULTIPAGE,
KGUISTRING_PRINTRANGE,
KGUISTRING_COPIES,

KGUISTRING_NUMSTRINGS
};
...
public:
	...
	static void SetLanguage(unsigned int lang);
	static void SetString(unsigned int num,kGUIString *string) {m_strings[num].SetString(string);}
	static void SetString(unsigned int num,const char *string) {m_strings[num].SetString(string);}
...
//	static volatile bool m_inupdate;	/* stop reentry */
//	static volatile bool m_indraw;	/* stop reentry */
//	static volatile bool m_inthread;	/* stop reentry */
	...
	static kGUIString m_strings[KGUISTRING_NUMSTRINGS];
